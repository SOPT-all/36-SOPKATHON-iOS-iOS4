...
final class MypageRecruitListCell: UITableViewCell {
    ...
    func dataBind(activity: Activity) {
        ...
        if let startDate = inputFormatter.date(from: activity.startTime),
           let endDate = inputFormatter.date(from: activity.endTime) {
            dateLabel.text = "\(outputFormatter.string(from: startDate)) ~ \(outputFormatter.string(from: endDate))"
        } else {
            dateLabel.text = "날짜 오류"
        }
        locationNameLabel.text = activity.region
        openerAndFeeLabel.text = ""
        
        filterLabel.text = "농업"
        
        if let imageUrl = activity.imageUrl, let url = URL(string: imageUrl) {
            recruitImageView.kf.setImage(with: url, placeholder: UIImage(named: "icPlaceholder"))
        } else {
            recruitImageView.image = UIImage(resource: .assetApple)
        }
    }
